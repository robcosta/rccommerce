...
public class UserServiceTests {
	...
	public void loadUserByUsernameShouldReturnUserDetailsWhenUserExist() {
		Mockito.when(repository.searchUserAndRolesByEmail(existsUsername)).thenReturn(userDetails);
	...
	public void loadUserByUsernameShouldTrowUsernameNotFoundExceptionWhenDoesNotExistUser() {
		Mockito.when(repository.searchUserAndRolesByEmail(nonExistsUsername)).thenReturn(List.of());
	...
	public void authenticatedShouldReturnUserWhenUserExixts() {
		Mockito.when(userUtil.getLoggerUsername()).thenReturn(existsUsername);
		Mockito.when(repository.findByEmail(existsUsername)).thenReturn(Optional.of(user));
	...
	public void findAllShouldReturnPagedUserMinDTOWhenEmptyNameAndEmail() {
		...
		Assertions.assertEquals(result.toList().get(0).getName(), existsNameUser);
	...
	public void findAllShouldfindAllShouldTrhowResouceNotFoundExceptionWhenDoesNotExistsName() {
		Mockito.when(repository.searchAll(nonExistsNameUser,emptyEmail, pageable)).thenReturn(new PageImpl<>(List.of()));
	...
	public void findAllShouldTrhowResouceNotFoundExceptionWhenDoesNotExistsEmail() {
		Mockito.when(repository.searchAll(emptyNameUser,nonExistsEmail, pageable)).thenReturn(new PageImpl<>(List.of()));
	...
	public void findAllShouldTrhowResouceNotFoundExceptionWhenDoesNotExistsNameAndEmail() {
		Mockito.when(repository.searchAll(nonExistsNameUser,nonExistsEmail, pageable)).thenReturn(new PageImpl<>(List.of()));
	...
	public void findByEmailShouldThrowResourceNotFoundExceptionWhenDoesNotExistsEmail() {
		Mockito.doThrow(ResourceNotFoundException.class).when(repository).findByEmail(nonExistsEmail);
